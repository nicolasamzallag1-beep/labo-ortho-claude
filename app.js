...
function render() {
    ...
    if (currentGame === 1) {
        body.innerHTML = `<p>${q.text.replace('___', '<span style="color:#6ee7b7">_______</span>')}</p>
            <input id="ans" class="input-field" placeholder="Écris ta réponse ici..." autofocus />
            <button class="btn primary" style="width:100%" onclick="check1()">Valider</button>`;
    } else if (currentGame === 2) {
        body.innerHTML = `<p style="margin-bottom:20px">${q.phrase}</p>` + q.reformulations.map(r => 
        ...
        if (!existing || existing.phase !== 'input') {
            body.innerHTML = `<p>Mémorise ce mot :</p><h2 style="color:#6ee7b7; font-size:50px; text-align:center">${q.word}</h2>
                <button class="btn primary" style="width:100%; margin-top:30px" onclick="startInput3()">J'ai mémorisé</button>`;
            speak(q.word);
        } else {
        ...
        body.innerHTML = `<p style="font-style:italic; text-align:center; font-size:28px">"${q}"</p>
            <div style="display:flex; gap:10px; margin-top:30px">
                <button class="btn primary" style="flex:1" onclick="check4(true)">Réussi ✅</button>
                <button class="btn muted" style="flex:1" onclick="check4(false)">À retravailler ❌</button>
            </div>`;
    }
}
...
function showEnd() {
    document.getElementById('modal-body').innerHTML = `<div style="text-align:center"><h2>Session terminée !</h2>
        <p style="font-size:40px; color:#6ee7b7">${sessionScore} / ${MAX_QUESTIONS}</p>
...
function loadStats() {
    ...
    document.getElementById('progress-list').innerHTML = [1,2,3,4].map(i => 
        `<div style="background:#1e293b; padding:15px; border-radius:10px; text-align:center; flex:1">
            Défi ${i}<br><b style="color:#6ee7b7; font-size:20px">${stats[i]}/10</b>
        </div>`).join('');
}
