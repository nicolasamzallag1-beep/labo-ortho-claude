...
function playSuccess() { playSound(800, 'sine', 0.2); setTimeout(() => playSound(1200, 'sine', 0.3), 100); }
...
function render() {
    ...
    if (currentGame === 1) {
        body.innerHTML = `<p>${q.text.replace('___', '<span style="color:#6ee7b7">_______</span>')}</p>
            ...
            <button class="btn primary" style="width:100%; max-width:500px" onclick="check1()">Valider</button>`;
    } else if (currentGame === 2) {
        body.innerHTML = `<p style="margin-bottom:30px">${q.phrase}</p>` + q.reformulations.map(r => 
        ...
        if (!existing || existing.phase !== 'input') {
            body.innerHTML = `<p>Mémorise ce mot :</p><h2 style="color:#6ee7b7; font-size:70px; margin:30px 0">${q.word}</h2>
                <button class="btn primary" style="width:100%; max-width:500px" onclick="startInput3()">J'ai mémorisé</button>`;
            speak(q.word);
        } else {
            body.innerHTML = `<p>Quel était le mot ?</p><input id="ans" class="input-field" autofocus />
                <button class="btn primary" style="width:100%; max-width:500px" onclick="check3()">Valider</button>`;
        }
    } else if (currentGame === 4) {
        body.innerHTML = `<p style="font-style:italic; font-size:36px; margin-bottom:40px">"${q}"</p>
...
function showEnd() {
    ...
    body.innerHTML = `<div style="text-align:center"><h2>Session terminée !</h2>
        <p style="font-size:60px; color:#6ee7b7; margin:30px 0">${sessionScore} / ${MAX_QUESTIONS}</p>
